...
class CompraSal(NewCoreLib.NewCoreLib):
    ...
    def __init__(self):
        """
        Inicializa o serviço do playwright e executa o navegador (firefox).
        :return: None
        """
    ...
    def comprasal(self, name: str) -> None:
        ...
        while page <= math.ceil((records or self.API_PAGE_SIZE) / self.API_PAGE_SIZE):
            ...
            search_response = self.make_request(
                url=f"{self.API_URL}/proveedores",
                method="POST",
                data=data
            )
        ...
        for key, row in enumerate(rows):
            ...
            if data_response is not None and data_response.status_code == 200:
                ...
                nomeComercial = str(data_result["nombreComercial"]).upper()
        ...
        console(f"\n{records} resultados encontrados.\n\n")
        write_results(json.dumps({"data": results}, ensure_ascii=False))
        # console(json.dumps({"data": results}, ensure_ascii=False, indent=4))
    ...
    def make_request(self, url, method="GET", data=None) -> requests.Response | None:
        ...
        while attempts < self.MAX_RETRIES:
            try:
                ...
                attempts += 1
                error(f"Tempo de espera da solicitação excedido - tentativa {attempts} de {self.MAX_RETRIES}")
                sleep(self.DEFAULT_SLEEP)
                continue
            except RequestException:
                attempts += 1
                error(f"Erro na solicitação - tentativa {attempts} de {self.MAX_RETRIES}")
                sleep(self.DEFAULT_SLEEP)
                continue
